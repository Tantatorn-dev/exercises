...
lastDigit n = if n < 0 then mod (0 - n) 10 else mod n 10
...
-- function.
minmax x y z = (tripletMax x y z) - (tripletMin x y z)
  where
    tripletMax x y z = max (max x y) (max y z)
    tripletMin x y z = min (min x y) (min y z)
...
strSum str = sum (map strToInt (words str))
  where
...
lowerAndGreater n list =
  show n
    ++ " is greater than "
    ++ show (countGreater n list 0)
    ++ " elements and lower than "
    ++ show (countLesser n list 0)
    ++ " elements"
  where
    len = length list

    countGreater :: Integer -> [Integer] -> Int -> Integer
    countGreater num arr i
      | i == len = 0
      | num > (arr !! i) = 1 + countGreater n arr (i + 1)
      | otherwise = countGreater num arr (i + 1)

    countLesser :: Integer -> [Integer] -> Int -> Integer
    countLesser num arr i
      | i == len = 0
      | num < (arr !! i) = 1 + countLesser n arr (i + 1)
      | otherwise = countLesser num arr (i + 1)